...
class H2TSparsityTrainer():
    ...
    def __init__(
        self,
        cfg: CfgNode, h2t_sparse_mode: str,
        model: nn.Module,
        evaluator: Evaluator,
        device: torch.device,
    ) -> None:
        self.cfg = cfg
        self.h2t_sparse_mode = h2t_sparse_mode
        self.model = model
        self.device = device

        # Fix learned prompts
        if h2t_sparse_mode in ['compress', 'featselect']:
            for k, v in model.named_parameters():
                if v.requires_grad and 'prompt' in k:
    ...
    def forward_one_batch(self, inputs, targets, is_train, lrp_coef=0.0,
                          feat_select_ids=None):
        ...
        with torch.set_grad_enabled(is_train):
            outputs = self.model(inputs, feat_select_ids=feat_select_ids)  # (batchsize, num_cls)
            if self.cfg.DBG:
                logger.info(
            ...
            if lrp_coef > 0.0:
                lrp_norm = torch.norm(
                        torch.norm(self.model.head.last_layer.weight, p=2, dim=0),
                        p=1)
                loss += lrp_coef * lrp_norm
    ...
    def train_classifier(self, train_loader, val_loader, test_loader,
                         lrp_coef=0.0, feat_select_ids=None):
        """
        ...
        for epoch in range(total_epoch):
            ...
            for idx, input_data in enumerate(train_loader):
                ...
                train_loss, _ = self.forward_one_batch(
                        X, targets, True, lrp_coef=lrp_coef,
                        feat_select_ids=feat_select_ids)
            ...
            self.eval_classifier(val_loader, "val", epoch == total_epoch - 1,
                    feat_select_ids=feat_select_ids)
            if test_loader is not None and epoch == total_epoch-1:
